...
if [ "$SYNC_LOCALHOST_KUBECONFIG" = "true" ] && [ -d "/usr/local/share/kube-localhost" ]; then
    ...
    sudo bash -c 'echo "192.168.65.254 kubernetes.default.svc" | tee -a /etc/hosts' > /dev/null
    sudo chown -R $(id -u) $HOME/.kube
    sed -i -e "s/localhost/kubernetes.default.svc/g" $HOME/.kube/config
    sed -i -e "s/127.0.0.1/kubernetes.default.svc/g" $HOME/.kube/config
